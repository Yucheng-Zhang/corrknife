...
int relwrap(double *dxyz, double *blen) {
  ...
  for (ii = 0; ii < 3; ii++) {
    if (dxyz[ii] >= blen[ii] / 2)
      dxyz[ii] = dxyz[ii] - blen[ii];
    if (dxyz[ii] < -blen[ii] / 2)
      dxyz[ii] = dxyz[ii] + blen[ii];
  }
  return 1;
...
void corr2d(double *xc_2d, double *p1, long fp1, long np1, double *p2, long fp2,
  ...
  int ppmin, ppmax, qqmin, qqmax, rrmin,
      rrmax;      // To figure of the range of grid span
  ...
  for (iq = fp1; iq < np1; iq++) {
    ...
    xyzp1[0] = p1[iq * nattr];
    xyzp1[1] = p1[iq * nattr + 1];
    xyzp1[2] = p1[iq * nattr + 2];
        ...
        for (rr = rrmin; rr < rrmax; rr++) {
          ...
          if (hoc[p * nhocells * nhocells + q * nhocells + r] != -1) {
            ...
            while (1) { // while follow the chains
              ...
              check = find_bin(xyzp1, xyzp2, samp, nbins, rlim, los, pbc, blen,
                               &indx, &indy);

              if (check != -1 && indx < nbins[0] && indy < nbins[1] &&
                  indx >= 0 && indy >= 0) {
...
int find_bin(double *axyz, double *bxyz, int samp, int *nsamp,
  ...
  if (los == 0 || los == 2 ||
      los == 3) { // the los is along the mid/first/second galaxy co-ordinate
    for (ii = 0; ii < 3; ii++) {
      difxyz[ii] = axyz[ii] - bxyz[ii];
      if (los == 0)
        losxyz[ii] = 0.5 * (axyz[ii] + bxyz[ii]); // los is along the mid point
      else if (los == 2)
        losxyz[ii] = axyz[ii]; // los is along the first galaxy
      else
        losxyz[ii] = bxyz[ii]; // los is along the second galaxy
  ...
  if (xsep > sampbound[0] && xsep < sampbound[1] && ysep > sampbound[2] &&
      ysep < sampbound[3]) {
    int xbin_n =
        floor((xsep - sampbound[0]) * nsamp[0] / (sampbound[1] - sampbound[0]));
    int ybin_n =
        floor((ysep - sampbound[2]) * nsamp[1] / (sampbound[3] - sampbound[2]));
    if (xbin_n == nsamp[0])
      xbin_n -= 1;
    if (ybin_n == nsamp[1])
      ybin_n -= 1;
